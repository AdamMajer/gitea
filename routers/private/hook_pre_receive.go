...
func preReceiveBranch(ctx *preReceiveContext, oldCommitID, newCommitID string, refFullName git.RefName) {
	...
	if branchName == repo.DefaultBranch && newCommitID == gitRepo.Hash.NewEmpty().String() {
	...
	if newCommitID == gitRepo.Hash.NewEmpty().String() {
	...
	if oldCommitID != gitRepo.Hash.NewEmpty().String() {
