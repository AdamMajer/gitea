...
func (pr *PullRequest) ConvertToGithubPullRequest(ctx context.Context, doer *user_model.User) (err error) {
	if !pr.IsAgitFlow() {
		return nil
	}

	pr.Flow = PullRequestFlowGithub
	newBranch := strings.Replace(pr.HeadBranch, "/", "_", 1)
	if !pr.HeadRepo.CanCreateBranch() {
		return fmt.Errorf("Cannot create branch in repository: %d", pr.HeadRepoID)
	}
	...
	repo, err := git.OpenRepository(ctx, pr.HeadRepo.RepoPath())
	if err != nil {
		return err
	}
	defer repo.Close()
	err = repo.CreateBranch(newBranch, pr.HeadCommitID)
	if err != nil {
		return err
	}

	pr.HeadBranch = newBranch
	return pr.UpdateColsIfNotMerged(ctx, "flow", "head_branch")
}
