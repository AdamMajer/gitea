...
type HashType interface {
	...
	// NewEmpty is the hash of empty git
	NewEmpty() Hash
...
func EmptyHash(ht HashType) Hash {
	return ht.NewEmpty()
}
...
func MustIDFromString(h HashType, s string) Hash {
	b, _ := hex.DecodeString(s)
	return h.MustID(b)
}

func NewIDFromString(ht HashType, s string) (Hash, error) {
	s = strings.TrimSpace(s)
	if len(s) != ht.FullLength() {
		return EmptyHash(ht), fmt.Errorf("length must be %d: %s", ht.FullLength(), s)
	}
...
func HashFromString(hexHash string) (Hash, error) {
	switch len(hexHash) {
	case 40:
		return NewIDFromString(Sha1HashType{}, hexHash)
	case 64:
		return NewIDFromString(Sha256HashType{}, hexHash)
	}
